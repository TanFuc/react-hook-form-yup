...
const schema = yup.object().shape({
    ...
    password: yup.string().min(6, 'Mật khẩu ít nhất 6 ký tự').max(20, 'Mật khẩu tối đa 20 ký tự').required('Vui lòng nhập mật khẩu'),
...
const RegisterForm = () => {
    ...
    const onSubmit = async (data) => {
        try {
            const response = await apiLocal.post('/posts', data);
